...
def Q_learn(mdp, q, lr=.1, iters=100, eps = 0.5, interactive_fn=None):
    # Your code here
    # The Q[(s,a)] are initialized to zero in TabularQ
    # Draw an initial state at random
    s = mdp.init_state()
    for i in range(iters):
        # include this line in the iteration, where i is the iteration number
        # Take action using the epsilon greedy approach
        a = epsilon_greedy(q, s,eps)
        # Execute a and obtain a reward and new state
        r,s_prime = mdp.sim_transition(s,a)
        # Check if s is a terminal state then gamma = 0 and the target is just the reward
        if mdp.terminal(s):
            target = r   
        else:
            # Update Q
            target = r + mdp.discount_factor * value(q, s_prime)
        data = [(s_prime, a, target)]
        q.update(data,lr)
        s = s_prime
...
def Q_learn_batch(mdp, q, lr=.1, iters=100, eps=0.5,
    ...
    for i in range(iters):
        ...
        for episode_ind in range(n_episodes):
            reward, episode, animation = sim_episode(mdp, episode_length, lambda s: epsilon_greedy(q,s,eps), draw=False)
            for e in episode:
                all_experiences.append(e)
        all_q_targets = []
        for ex in all_experiences:  # each experience is a tuple (s,a,r,s_prime)
            if ex[3] == None:
                q_target = (ex[0],ex[1], ex[2]) # Each q_target is a tuple (s,a,t)
            else:
                s, a, r, s_prime = ex
                target = r + mdp.discount_factor * value(q, s_prime)
                q_target = (s,a,target)
...
###############################################################################
# The following part isn't completed due to technical problems arising from updates in the keras versions.
