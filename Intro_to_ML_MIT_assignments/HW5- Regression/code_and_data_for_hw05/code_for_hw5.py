...
def d_square_loss_th(x, y, th, th0):
    ...
    """
    return 2*d_lin_reg_th(x,th,th0)*(lin_reg(x, th, th0)-y)
...
def sgd(X, y, J, dJ, w0, step_size_fn, max_iter):
    ...
    fs = [J(X[:,i:i+1],y[:,i:i+1],w)]
    ws = [w0]
    # Iterations 
    for t in range(max_iter-1):
        step_size = step_size_fn(t)
        w_new = w - step_size * dJ(X[:,i:i+1],y[:,i:i+1],w)
        w = w_new
        ws.append(w)
        i = np.random.randint(n)
...
def sgdTest():
    ...
    sgd(X, y, J, dJ, cv([0,0]), lambda i: 0.1, 1000)
...
order = [1,2,3]
lambda2 = np.arange(0.01,0.11,0.01).tolist()
lambda3 = list(range(20,201,20))


# Q9C, Q9D: The minimum rmse and the corresoponding feature set polynomial order
# and lambda. 
...
for o in order:
    ...
    for i in range(2):
        for lam in lambda_list:
            lam_ind = lambda_list.index(lam) 
            score = xval_learning_alg(make_polynomial_feature_fun(o)(auto_data[i]), auto_values, lam, 10)
            rmse = (score * sigma)[0,0]
            table[rmse] = (i+1,o,lam)

feature_min,order_min, lam_min = table[min(table.keys())]
print(f'The minimum rmse is associated with the \n  feature: {feature_min}, order: {order_min},lambda: {lam_min}')         
 
# Q9E The minimum rmse and lam for feature1 and order 3           
table = {}
rmse_list = []
lambda_list = lambda3
for lam in lambda_list:
    lam_ind = lambda_list.index(lam) 
    score = xval_learning_alg(make_polynomial_feature_fun(3)(auto_data[0]), auto_values, lam, 10)
    rmse = (score * sigma)[0,0]
    table[rmse] = lam

rmse_min = min(table.keys())
lam_min = table[rmse_min]

print(f'list of minimum rmse and corresponding lam: [{rmse_min},{lam_min}]')
# print(list(table[min(rmse_list)]))
# print(min(rmse_list))