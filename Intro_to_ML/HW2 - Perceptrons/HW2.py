...
def perceptron(data, labels, params={}, hook=None):
    ...
    for t in range(T):
        ...
        if check:       
             break
        if hook: hook((th, th0[0,0]))
    return th, th0



# Visualization of Perceptron:
...
def averaged_perceptron(data, labels, params={}, hook=None):
    ''' Averaged preseptron algorithm. The algorithm averages all the obtained
        seperators th and offsets th, and returns and average seperator and offset.
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    ''' Evalutes the classifier. Obtains a linear seperator utilizing the 
        training data and tests the performance on the testing data. Return the
        fraction of errors the classifier made.
        
        Input
            learner: function, a learning algorithm
            data_train: d by n array, training data
            labels_train: 1 by n array, training labels
            data_test: d by n array, testing data
            labels_test: 1 by n array, testing labels
        
        Retruns
            normalized_score: float between 0.0 and 1.0, the fraction of errors.
    '''    
    # Produce a seperator and offset using the learner learning algorithm and
    # training data
    th,th0 = learner(data_train, labels_train, params={}, hook=None)
    
    # Evaluate the normalized score (# of error/# number of feature vectors)
    # of the testing data using th and th0.
    d, n = data_test.shape
    error_fraction = score(data_test, labels_test, th, th0)/n
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ''' Generated training and testing data it times. Each time evaluates the 
        error fraction and returns the average error fraction
        (a.k.a average classification accuracy).
        
        Input
            learner: function, learning algorithm
            data_gen: function, generates a tuple (data,labels)
            n_train: int, number of training data samples
            n_test: int, number of testing data samples
            
        Prameters
            data_train: d by n_train array, the training data
            labels_train: 1 by n_train array, the training labels
            data_train: d by n_test array, the testing data
            labels_train: 1 by n_test array, the testing labels
        
        Retruns
            The averages error fraction (a.k.a average classification accuracy),
    ...
    for i in range(it):
        ...
        # Ealuate the classifier
        error_fraction += eval_classifier(learner, data_train, labels_train, data_test, labels_test)
        
    # Return the average the error fraction
    return error_fraction/it
...
def xval_learning_alg(learner, data, labels, k):
    ''' Cross evaluation of the learning algorithm.
        The algorithm splits the data set into k parts. For j = 1,...,k 
        removes the j'th part from the data. Uses the data left as a training data,
        obtains a linear seperator and evaluates the accuracy of the associated classifier 
        employing the j'th part as the testing data.
        
        Input
            learner: function, learning algorithm
            data: d by n array, the data set
            labels: 1 by n array, the associated labels
            k: int, number of partitions
        
        Returns
            Averaged score: float between 0 and 1
    '''
    # Divide the data into k parts
    split_data = np.array_split(data, k,axis = 1)
    split_labels = np.array_split(labels,k,axis = 1)
    scores = []
    # For j = 1 to k gather all the data except the j'th split data set.
    for j in range(k):
        data_minus_j_list = split_data[:]
        data_minus_j_list.pop(j)
        data_minus_j = np.concatenate(tuple(data_minus_j_list),axis = 1)
        
        
        labels_minus_j_list = split_labels[:]
        labels_minus_j_list.pop(j)
        labels_minus_j = np.concatenate(tuple(labels_minus_j_list),axis = 1)
        
        # Run the learning algorithm on concatenated data set (not including the j'th split data set)
        th,th0 = learner(data_minus_j, labels_minus_j, params={}, hook=None)
        size = split_data[j].shape[1]
        score_j = score(split_data[j], split_labels[j], th, th0)/size
...
#prob = 0.25
#print(eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=prob), 20, 20, 5))
#print(eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=prob), 20, 20, 5))
